...
{LGSE6.3 DDTS Test Manual Ver.2.1}

\subtitle
{for O18/M16PP - WebOS 4.0} % (optional)


\author[Sunha Jeon, JongKun Kim, Kichul Kim] % (optional, use only with lots of authors)
{Sunha Jeon (sunha.jeon@lge.com)\\JongKun Kim (jkun.kim@lge.com)\\Kichul Kim (kichul.kim@lge.com)}
...
\institute[IPT team, SIC lab., LG Electronics] % (optional, but mostly needed)
{
	...
	Release Link: (http://collab.lge.com/main/display/LGSE/LGSE6.3+DDTS+TEST+SPEC)
}
...
% \pgfdeclareimage[height=0.32cm]{university-logo}{university-logo-filename}
...
\begin{document}
	...
	\begin{frame}[t]{History}
		\begin{itemize}
			\begin{scriptsize}
				\item 2017.07.12\_Ver.1 Init
				\item 2017.08.07\_Ver.2 AV test spec 변경, DRC figure 변경
			\end{scriptsize}
		\end{itemize}
	\end{frame}
	...
	\begin{frame}[t]{Dynamic Range Control}
		\begin{tiny}
			\begin{tabular}{@{}lccc@{}}
				...
				\multirow{14}{60mm}{
					\begin{itemize}
						...
						\begin{itemize}
							...
							\item 400Hz-4.5kHz에서 ref 신호와의 레벨 차이값이 6dBr 이상\\(ref신호<측정신호)
							\item 7kHz-20kHz에서 ref 신호와의 레벨 차이값이 -6dBr 이하\\(ref신호>측정신호)
						\end{itemize}
					\end{itemize}
				} \\
				Clearvoice & \color{black}{Off} & & \\
				...
				Limiter & \color{black}{Off} & & \\
				Sound Stage Expansion & \color{black}{Off} & & \\
				\color{blue}{Dynamic Range Control} & \color{blue}{On} & & \\
				Smart Sound Controller & \color{black}{Off} & & \\
				High Resolution & \color{black}{Off} & & \\
				User EQ & \color{black}{Off} & & \\
				Room EQ & \color{black}{Off} & & \\
